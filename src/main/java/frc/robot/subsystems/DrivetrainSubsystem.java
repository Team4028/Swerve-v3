...
public class DrivetrainSubsystem extends SwerveDrivetrain {
    ...
    public static final ITrajectoryConstraint[] CONSTRAINTS = { new MaxVelocityConstraint(12.0 * 12.0),
            new MaxAccelerationConstraint(15.0 * 12.0), new CentripetalAccelerationConstraint(25.0 * 12.0) };

    private static final double FRONT_LEFT_ANGLE_OFFSET = Math.toRadians(-45); //-45
    private static final double FRONT_RIGHT_ANGLE_OFFSET = Math.toRadians(-55); //-45
    private static final double BACK_LEFT_ANGLE_OFFSET = Math.toRadians(0+180); //-170
    private static final double BACK_RIGHT_ANGLE_OFFSET = Math.toRadians(-135);  //-135 55
